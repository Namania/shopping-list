...
class _CalculatorListState extends State<CalculatorList> {
  void addToCalculator(BuildContext context, String data) {
    ...
    if (value != null) {
      context.read<CalculatorBloc>().add(
        CalculatorAddWithoutArticleEvent(idList: widget.idList, price: value),
      );
    }
  }
  ...
  void handleDelete(BuildContext context, int value) {
    switch (value) {
      case 1:
        context.read<CalculatorBloc>().add(
          CalculatorResetWithEvent(),
        );
        break;
      case 0:
        context.read<CalculatorBloc>().add(
          CalculatorResetWithoutArticleEvent(),
        );
        break;
      case -1:
        context.read<CalculatorBloc>().add(
    ...
    return Scaffold(
                                ...
                                children: [
                                  Expanded(
                                    child: FilledButton.icon(
                                      ...
                                      onPressed:
                                          () => handleDelete(context, -1),
                                      label: Text(
                                        context.tr('calculator.delete.all'),
                                      ),
                                    ),
                                  ),
                                ],
      ...
      body: BlocBuilder<CalculatorBloc, CalculatorState>(
        ...
        builder: (context, state) {
          final data =
              widget.specificId
                  ? context.read<CalculatorBloc>().getCalculatorByListId(
                    widget.idList,
                  )
                  : context.read<CalculatorBloc>().getAllCalculator();
          final amount =
              widget.specificId
