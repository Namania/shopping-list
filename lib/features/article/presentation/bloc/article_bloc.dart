...
class ArticleBloc extends Bloc<ArticleEvent, ArticleState> {
  ...
  ArticleBloc({
    ...
    on<ArticleGetAllEvent>(
      (event, emit) => _onGetAll(event, emit),
    );
    on<AddArticleEvent>(
      (event, emit) => _onAddArticle(event, emit),
    );
    on<RemoveArticleEvent>(
      (event, emit) => _onRemoveArticle(event, emit),
    );
    on<ClearEvent>(
      (event, emit) => _onClear(event, emit),
    );
  ...
  Future<void> _onAddArticle(
    AddArticleEvent event,
    Emitter emit,
  ) async {
    emit(ArticleLoading());
    final result = await addArticle(
      AddArticleParams(article: event.article)
    );
  ...
  Future<void> _onRemoveArticle(
    ...
    final result = await removeArticle(
      RemoveArticleParams(article: event.article)
    );
